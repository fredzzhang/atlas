...
from src.utils import cosine_lr, get_n_shots_preds, TIPWrapper, LPPWrapper, IndexWrapper, _RepeatSampler
...
def train(task_vectors, args, comp_acc={}):
    ...
    if type(args.subsample) == int:
        if os.path.isfile(f"{args.save}/{target_dataset}/{args.subsample}_shots_{args.seed}.pt") and args.seed == 1:
            ...
            to_keep = get_n_shots_preds(dataset.train_dataset, args.subsample, classification_head.out_features, args)
...
if __name__ == "__main__":
    ...
    if type(args.subsample) == float:
        ...
        args.target_datasets = {k:0 for k,v in args.target_datasets.items()}#10 epochs for few-shots using ViTs. 30 epochs is better for ResNets.
