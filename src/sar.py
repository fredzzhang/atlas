"""
Adapted from SAR to be compatible with lightning.Fabric & TVs coeficients
...
class SAR(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, fabric, steps=1, episodic=False, margin_e0=0.4*math.log(1000), reset_constant_em=0.2):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs, ema, reset_flag = forward_and_adapt_sar(x, self.model, self.fabric, self.optimizer, self.margin_e0, self.reset_constant_em, self.ema)
            if reset_flag:
                self.reset()
...
def forward_and_adapt_sar(x, model, fabric, optimizer, margin, reset_constant, ema):
