...
from src.sar import SAR, SAM, collect_params
...
def train(task_vectors, args):
    ...
    if args.tip_ft:
        if args.lp:
            ...
            acc = eval_single_dataset(image_encoder, test_dataset.split('Val')[0], args, adapter=adapter, alpha_vec=alpha_vec, test=True)
            acc["alpha_vec"] = best_alpha
        else:
            adapter = best_adapter
            acc = eval_single_dataset(image_encoder, test_dataset.split('Val')[0], args, adapter=adapter, beta_alpha=adapter.beta_alpha, labels_cache=labels_cache, test=True)
