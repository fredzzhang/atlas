...
def build_gym(args):
    ...
    for filename in sorted(os.listdir(args.task_dir)):
        if (
            # filename.startswith("ai2_arc")
            filename.startswith("commonsense_qa")
            or filename.startswith("quail")
            or filename.startswith("sciq")
        ):
    ...
    if len(all_failed_tasks) > 0:
        print(
            "Please try the following tasks later by running individual files: {}".format(
                all_failed_tasks
            )
        )
    else:
