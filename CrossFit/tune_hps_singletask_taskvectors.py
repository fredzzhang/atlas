...
from run_singletask_taskvectors import run
...
def main():
    ...
    parser.add_argument(
        "--warmup_steps", default=500, type=int, help="Linear warmup over warmup_steps."
    )
    parser.add_argument(
        "--total_steps",
        default=100000,
    ...
    df = pd.DataFrame(
        columns=["prefix", "lr", "bsz", "dev_performance", "test_performance"]
    )
        ...
        for lr in args.learning_rate_list:
            for bsz in args.bsz_list:
                ...
                if args.memory_efficient:
                    ...
                    logger.info(
                        "Using memory efficient training: batch size={}, gradient accumulation={}, effective batch size={}".format(
                            2, int(bsz / 2), bsz
                        )
                    )
                ...
                dev_performance, test_performance = run(args, logger, prefix)

                logger.info(
                    "prefix={}, lr={}, bsz={}, dev_performance={}, test_performance={}".format(
